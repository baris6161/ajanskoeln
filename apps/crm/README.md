...
**Vercel (ein Projekt):** Im Dashboard **Root Directory** auf `apps/crm` setzen, damit Next erkannt wird. **Install Command:** `cd ../.. && npm install` (oder `npm ci`). **Build Command:** `cd ../.. && npm run build`. Umgebungsvariablen wie bisher im CRM-Projekt hinterlegen und die Domain an dieses Deployment binden.
