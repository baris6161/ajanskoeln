        ...
        <div className="mt-14 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
          {t.usp.items.map((item, i) => {
            ...
            return (
              <article
                key={i}
                className={`reveal group rounded-2xl border border-border bg-card p-6 transition-all duration-300 hover:-translate-y-1 hover:border-accent/40 hover:shadow-[var(--shadow-elegant)]${i >= 4 ? " hidden sm:block" : ""}`}
